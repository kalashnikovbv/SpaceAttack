...
[AddComponentMenu("Mobile/Joystick")]
			...
			for (int i = 0; i < count; i++)
			{
				...
				if (lastFingerId == touch.fingerId)
				{
					...
					else
					{
						...
						Rect pix = gui.pixelInset;
						
						if (!isVertical) pix.x = Mathf.Clamp(guiTouchPos.x, guiBoundary.min.x, guiBoundary.max.x);
						if (!isHorizontal) pix.y = Mathf.Clamp(guiTouchPos.y, guiBoundary.min.y, guiBoundary.max.y);
						gui.pixelInset = pix;
					}
